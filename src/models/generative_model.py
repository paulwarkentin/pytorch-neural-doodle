...
class GenerativeModel(nn.Module):
    """A PyTorch module that implements the neural network for generating a doodle.

	It is based on the VGG 19 network (https://arxiv.org/abs/1409.1556).
    ...
    def __init__(self, input_channels = 3):
		...
		self.map1 = nn.MaxPool2d(1, count_include_pad=False)
		self.map2 = nn.MaxPool2d(2, count_include_pad=False)
		self.map3 = nn.MaxPool2d(4, count_include_pad=False)
		self.map4 = nn.MaxPool2d(8, count_include_pad=False)
		self.map5 = nn.MaxPool2d(16, count_include_pad=False)
	...
	def initialize(self, filename):
        ...
        for name in data:
            ...
            module.weight.data = torch.from_numpy(data[name][0])
            module.bias.data = torch.from_numpy(data[name][1])


    def forward(self, image_input, map_input):
        """Do forward calculations of the network.

		Arguments:
			image_input: Original input image.
			map_input: Style input image.
		...
		for block_ii, num_convs in enumerate([2, 2, 4, 4, 4]):
			...
			for conv_ii in range(num_convs):
				...
				sem_name = "{}_{}".format(sem_block_name, conv_ii + 1)
				map_pooled = getattr(self, map_block_name)(map_input)
				y = torch.cat((x, map_pooled))
				outputs[sem_name] = y

				nn_name = "{}_{}".format(nn_block_name, conv_ii + 1)
				y = getattr(self, nn_name)(y)
				outputs[nn_name] = y
